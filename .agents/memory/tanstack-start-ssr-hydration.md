...
In development, if a warning remains after client-only state is made SSR-safe, inspect the exact differing attributes. TanStack's dev source annotations and browser extensions can add non-production attributes such as `data-tsd-source` or caret styles.
